...
export function setupChatRoute(app, client, upload) {
  // Upload handler middleware with improved error handling
  const handleUpload = (req, res, next) => {
    upload.array("files", 5)(req, res, (err) => {
      if (err) {
        ...
        if (err.message.includes('File type') || err.message.includes('File must have')) {
  ...
  try {
    ...
    // Check message size before sending (Vertex AI has 30MB limit)
    const estimatedSize = JSON.stringify(userContent).length;
    const maxSize = 25 * 1024 * 1024; // 25MB to be safe (leaves room for base64 overhead)
    if (estimatedSize > maxSize) {
      const sizeMB = (estimatedSize / 1024 / 1024).toFixed(1);
      throw new Error(`Message too large (${sizeMB}MB). Vertex AI has a 30MB limit. Try uploading smaller files or fewer files at once.`);
    }
